...
import { IKudugridProps } from './IKudugridProps';
...
export default class Kudugrid extends React.Component<IKudugridProps, IDetailsListDocumentsExampleState> {
  private _selection: Selection;
  private _allItems: any[];

  constructor(props: IKudugridProps) {
    super(props);
    ...
    const columns: IColumn[] = [
      ...
      },
      {
        ...
        onColumnClick: this._onColumnClick,
        onRender: (item: any) => {
          return <span>{item.Category.CategoryName}</span>;
        },
      },
    ];
  ...
  private _onChangeText = (ev: React.FormEvent<HTMLInputElement | HTMLTextAreaElement>, text: string): void => {
    ...
    this.setState({
      items: text ? this._allItems.filter(i => i.ProductName.toLowerCase().indexOf(text) > -1) : this._allItems,
    });
  };

  private _onItemInvoked(item: any): void {
    alert(`Item invoked: ${item.ProductName}`);
  }
